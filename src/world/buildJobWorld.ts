...
import { buildRestaurant } from './buildRestaurant.ts';
import { emptyWorldTextures } from './textures.ts';
import type { WorldTextures } from './textures.ts';

export function buildJobWorld(app: AppBase, job: JobDef, textures: WorldTextures = emptyWorldTextures()): BuiltWorld {
    if (job.environment === 'mining-facility') {
        return buildMine(app, job, textures.rockWall);
    }
    if (job.environment === 'space-burger') {
        return buildRestaurant(app, job);
    }
